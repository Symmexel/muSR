...
H.add_all_interactions(H.add_fluorine(np.array([-a/2, 0, a/4])))
H.add_all_interactions(H.add_fluorine(np.array([-a/2, 0, -a/4])))
H.add_all_interactions(H.add_fluorine(np.array([0, a/2, a/4])))
H.add_all_interactions(H.add_fluorine(np.array([0, a/2, -a/4])))
H.add_all_interactions(H.add_fluorine(np.array([0, -a/2, a/4])))
H.add_all_interactions(H.add_fluorine(np.array([0, -a/2, -a/4])))
...
H_scaled.add_all_interactions(H_scaled.add_fluorine(scaling*np.array([-a/2, 0, a/4])))
H_scaled.add_all_interactions(H_scaled.add_fluorine(scaling*np.array([-a/2, 0, -a/4])))
H_scaled.add_all_interactions(H_scaled.add_fluorine(scaling*np.array([0, a/2, a/4])))
H_scaled.add_all_interactions(H_scaled.add_fluorine(scaling*np.array([0, a/2, -a/4])))
H_scaled.add_all_interactions(H_scaled.add_fluorine(scaling*np.array([0, -a/2, a/4])))
H_scaled.add_all_interactions(H_scaled.add_fluorine(scaling*np.array([0, -a/2, -a/4])))
m_scaled = Model(H_scaled.build())
...
plt.plot(ts*1e6, m.polarisation(ts), linestyle='--', color='k')
# plt.plot(ts*1e6, Model(H.build()).polarisation(ts), linestyle='--', color='k')
# plt.plot(ts*1e6, Model(H_scaled.build()).polarisation(ts), color='blue')
plt.plot(ts*1e6, m_scaled.polarisation(ts), color='blue')
plt.xlabel('Time, $t$ / $\mathrm{{\mu}s^{-1}}$')
plt.ylabel('Observed polarisation, $D_{z}(t)$')
plt.tight_layout()

# Model(H.build()).plot_cosine_amplitudes(plt.gca())
